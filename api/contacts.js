...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    try {
      ...
      // Her kişi için firma ve görüşme bilgisini al
      const result = [];
      for (const p of persons) {
        const [pc] = await sql`
          SELECT pc.title, pc.phone, pc.gsm, pc.fax, pc.email, pc.web,
                 c.name as company_name, c.sector, c.address, c.id as company_id
          FROM person_companies pc
          JOIN companies c ON c.id = pc.company_id
          WHERE pc.person_id = ${p.id} AND pc.is_primary = true
          LIMIT 1
        `;
        const [m] = await sql`
          SELECT category, notes, next_action, next_action_date, id as meeting_id
          FROM meetings
